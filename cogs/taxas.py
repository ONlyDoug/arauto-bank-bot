...
# Função auxiliar format_list_for_embed atualizada (com nome de campo opcional)
def format_list_for_embed(member_data, limit=40, field_name="Membros"):
    if not member_data:
    ...
    # Proteção contra tamanhos excessivos (description do embed tem limite ~4096)
    if len(text) > 4000:
        text = text[:3996] + "\n..."
    return text
...
class Taxas(commands.Cog):
    def __init__(self, bot):
        ...
        print("Módulo de Taxas v3.1 (Relatório Detalhado no Comando Restaurado) pronto.")
    ...
    async def _update_report_message(self, canal: discord.TextChannel, config_key: str, embed: discord.Embed):
        ...
        try:
            ...
            # Se for demasiado longo, tenta enviar uma versão de erro
            try:
