...
class Engajamento(commands.Cog):
    ...
    async def recompensar_voz(self):
        try:
            recompensa_voz = int(self.bot.db_manager.get_config_value('recompensa_voz', '0'))
            limite_voz_minutos = int(self.bot.db_manager.get_config_value('limite_voz', '0'))
            ...
            print(f"Erro na tarefa de recompensar_voz: {e}")
    ...
    async def on_message(self, message):
        ...
        recompensa_chat = int(self.bot.db_manager.get_config_value('recompensa_chat', '0'))
        limite_chat = int(self.bot.db_manager.get_config_value('limite_chat', '0'))
        cooldown_chat = int(self.bot.db_manager.get_config_value('cooldown_chat', '60'))
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        canal_anuncios_id = self.bot.db_manager.get_config_value('canal_anuncios', '0')
        recompensa_reacao = int(self.bot.db_manager.get_config_value('recompensa_reacao', '0'))
    ...
    async def enviar_mensagem_engajamento(self):
        try:
            canal_id_str = self.bot.db_manager.get_config_value("canal_batepapo", '0')
