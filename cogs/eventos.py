...
class EventoView(discord.ui.View):
    ...
    async def atualizar_mensagem(self, interaction: discord.Interaction):
        ...
        class DetalhesEventoModal(discord.ui.Modal, title='Detalhes Essenciais do Evento'):
            ...
            async def on_submit(self, interaction: discord.Interaction):
                try:
                    naive = datetime.datetime.strptime(self.data_hora.value, '%Y-%m-%d %H:%M')
                    aware = naive.replace(tzinfo=datetime.timezone.utc).astimezone()
                    self.view.evento_data['data_evento'] = aware
                    self.view.evento_data['nome'] = self.nome.value
                    self.view.evento_data['tipo_evento'] = self.tipo_evento.value
                    self.view.evento_data['descricao'] = self.descricao.value
                    await self.view.atualizar_preview(interaction)
        ...
        class CriacaoEventoView(discord.ui.View):
            ...
            async def publicar_evento(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                resultado = await self.bot.db_manager.execute_query(
                    ...
                    self.evento_data['nome'],
                    self.evento_data.get('descricao'),
                    self.evento_data.get('tipo_evento'),
                    self.evento_data['data_evento'],
                    self.evento_data.get('recompensa', 0),
                    self.evento_data.get('max_participantes'),
                    interaction.user.id,
                    cargo_id,
                    fetch="one",
                )
        ...
        class Eventos(commands.Cog):
            ...
            async def agendarevento(self, ctx: commands.Context):
                ...
                if str(ctx.channel.id) != canal_planejamento_id_str:
                    canal_planejamento = self.bot.get_channel(int(canal_planejamento_id_str)) if canal_planejamento_id_str != '0' else None
                ...
                embed = discord.Embed(
                    title="Assistente de Criação de Eventos",
                    description="Use os botões abaixo para configurar o seu evento. A pré-visualização será atualizada em tempo real.",
